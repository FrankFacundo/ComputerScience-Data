...
\begin{multicols*}{3}
    ...
    \begin{description}
        \item[Expected Value] (a.k.a.~\emph{mean}, \emph{expectation}, or \emph{average}) is a weighted average of the possible outcomes of our random variable. Mathematically, if $x_1, x_2, x_3, \dots$ are all of the distinct possible values that $X$ can take, the expected value of $X$ is
              ...
              \end{center}
              On a discrete probability space $(\Omega,\mathcal{A},\mathbb{P})$ with a random variable
              $X : \Omega \to \mathbb{R}$, a more general way to write expectation is
              \[
                  \mathbb{E}[X] = \sum_{\omega \in \Omega} X(\omega)\,\mathbb{P}(\{\omega\}).
              \]
              In particular, for two random variables $X$ and $Y$,
              \[
                  \mathbb{E}[X+Y] = \sum_{\omega \in \Omega} \bigl(X(\omega)+Y(\omega)\bigr)\,\mathbb{P}(\{\omega\}).
              \]
    ...
    \begin{description}
        ...
        \item[How do I find the expected value of a CRV?]
              For a random variable $X : \Omega \to \mathbb{R}$ on a probability space
              $(\Omega,\mathcal{A},\mathbb{P})$, the general (measure-theoretic) definition of
              expectation is the Lebesgue integral
              \[
                  \mathbb{E}[X] = \int_{\Omega} X(\omega)\,d\mathbb{P}(\omega).
              \]
              If $X$ admits a density $f$ with respect to the Lebesgue measure on $\mathbb{R}$,
              this reduces to the usual “$x$ times the PDF” formula:
              \[
    ...
    \begin{description}
        \item[Expected value of a function of an r.v.]
              The expected value of $X$ on a probability space $(\Omega,\mathcal{A},\mathbb{P})$
              is, in full generality,
              \[
                  E(X) = \int_{\Omega} X(\omega)\,d\mathbb{P}(\omega).
              \]
              In the discrete and continuous cases this becomes
              \[
                  E(X) = \sum_x x\,P(X=x) \quad \textnormal{(for discrete $X$)},
              \]
              \[
                  E(X) = \int_{-\infty}^{\infty} x f(x)\,dx \quad \textnormal{(for continuous $X$)}.
              \]

              The \textbf{Law of the Unconscious Statistician (LOTUS)} states that you can find
              the expected value of a \emph{function of a random variable}, $g(X)$, by
              integrating (or summing) $g$ composed with $X$:
              \[
                  E\bigl(g(X)\bigr)
                  = \int_{\Omega} g\bigl(X(\omega)\bigr)\,d\mathbb{P}(\omega)
              \]
              and, in the discrete and continuous special cases,
              \[
                  E(g(X)) = \sum_x g(x)\,P(X=x) \quad \textnormal{(for discrete $X$)},
              \]
              \[
                  E(g(X)) = \int_{-\infty}^{\infty} g(x) f(x)\,dx \quad \textnormal{(for continuous $X$)}.
              \]
    ...
    \section{Measure-Theoretic Foundations (FRENCH)}\smallskip \hrule height 2pt \smallskip

    \subsection{Espaces mesurés et espaces probabilisés}

    \textbf{Espace mesurable} : $(\Omega,\mathcal{A})$ où $\Omega$ est un ensemble et
    $\mathcal{A} \subset 2^\Omega = \mathcal{P}(\Omega)$ (l’ensemble des parties de $\Omega$)
    une \textbf{tribu} (ou $\sigma$-algèbre).

    \begin{itemize}
        \item $\Omega \in \mathcal{A}$, $\emptyset \in \mathcal{A}$,
        \item $A\in\mathcal{A} \Rightarrow A^c \in\mathcal{A}$,
        \item $(A_n)_{n\ge 1}\subset\mathcal{A} \Rightarrow \bigcup_{n\ge 1} A_n \in\mathcal{A}$ (donc aussi $\bigcap_{n\ge 1} A_n\in\mathcal{A}$).
    \end{itemize}

    \textbf{Boréliens sur $\mathbb{R}$} : on note $\mathcal{B}(\mathbb{R})$ la \textbf{tribu de Borel} sur $\mathbb{R}$, c’est-à-dire la plus petite tribu de parties de $\mathbb{R}$ contenant tous les ouverts (par exemple tous les intervalles ouverts).
    Les ensembles de $\mathcal{B}(\mathbb{R})$ sont appelés \textbf{boréliens} (ou ensembles boréliens).

    \textbf{Mesure} sur $(\Omega,\mathcal{A})$ : application $\mu:\mathcal{A}\to[0,\infty]$ telle que
    \begin{itemize}
        \item $\mu(\emptyset)=0$,
        \item (additivité dénombrable) pour $A_i$ disjoints :
              $\displaystyle \mu\!\left(\bigcup_{i=1}^\infty A_i\right) = \sum_{i=1}^\infty \mu(A_i)$.
    \end{itemize}

    \textbf{Espace probabilisé} : $(\Omega,\mathcal{A},\mathbb{P})$ où
    $\Omega$ est un ensemble,
    $\mathcal{A}\subset 2^\Omega$ une \textbf{tribu} (ou $\sigma$-algèbre),
    donc $(\Omega,\mathcal{A})$ est un espace mesurable,
    et $\mathbb{P} : \mathcal{A} \to [0,1]$ une mesure de probabilité telle que $\mathbb{P}(\Omega)=1$.


    \textbf{Propriétés de base} (pour $A,B\in\mathcal{A}$, $A\subset B$) :
    \[
        \mathbb{P}(A^c)=1-\mathbb{P}(A),\quad
        \mathbb{P}(A)\le \mathbb{P}(B),\quad
        \mathbb{P}\Big(\bigcup_{n}A_n\Big)\le \sum_n\mathbb{P}(A_n).
    \]
    \textbf{Continuité} : si $A_n\uparrow A$, alors $\mathbb{P}(A_n)\uparrow \mathbb{P}(A)$; si $A_n\downarrow A$, alors $\mathbb{P}(A_n)\downarrow \mathbb{P}(A)$.

    \subsection{Presque-sûr et égalité p.s.}

    \begin{itemize}
        \item Un événement $A$ a lieu \textbf{presque sûrement (p.s.)} si $\mathbb{P}(A)=1$.
        \item Deux v.a. $X,Y$ sont égales p.s. si $\mathbb{P}(X\neq Y)=0$ (on écrit $X=Y$ p.s.).
    \end{itemize}
    En théorie de la mesure, beaucoup de résultats sont valables \emph{à une modification p.s. près}.

    \subsection{Variables aléatoires et loi}

    \textbf{Variable aléatoire réelle} : application mesurable
    \[
        X:(\Omega,\mathcal{A})\to(\mathbb{R},\mathcal{B}(\mathbb{R})),\quad
        \omega \mapsto X(\omega).
    \]
    On note $X(\omega)$ la valeur prise par $X$ lorsque l’issue élémentaire est $\omega\in\Omega$.

    On définit, pour tout borélien $B\in\mathcal{B}(\mathbb{R})$,
    \[
        \{X\in B\} := X^{-1}(B)
        = \{\omega\in\Omega : X(\omega)\in B\} \in\mathcal{A}.
    \]

    \textbf{$\sigma$-algèbre engendrée} par $X$ : $\sigma(X)=\{X^{-1}(B):B\in\mathcal{B}(\mathbb{R})\}$.

    \textbf{Loi de $X$} : mesure image $\mathbb{P}_X$ sur $(\mathbb{R},\mathcal{B}(\mathbb{R}))$ définie par
    \[
        \mathbb{P}_X(B) = \mathbb{P}(X\in B)=\mathbb{P}\big(X^{-1}(B)\big).
    \]
    Les notions de PMF/PDF/CDF classiques sont des descriptions concrètes de $\mathbb{P}_X$.

    \textbf{Indépendance} :
    \begin{itemize}
        \item $X,Y$ indépendantes $\iff$ pour tous boréliens $A,B$ :
              $\mathbb{P}(X\in A,Y\in B)=\mathbb{P}_X(A)\mathbb{P}_Y(B)$.
        \item Équivalent : les $\sigma$-algèbres $\sigma(X)$ et $\sigma(Y)$ sont indépendantes.
    \end{itemize}

    \subsection{Espérance comme intégrale de Lebesgue}

    Pour une v.a. $X$ mesurable et $X\ge 0$ :
    \[
        \mathbb{E}(X)=\int_\Omega X(\omega)\,d\mathbb{P}(\omega)
    \]
    est définie comme la limite de l’intégrale de fonctions simples croissantes qui approchent $X$.

    Pour une v.a. intégrable ($\int |X|\,d\mathbb{P}<\infty$), on pose
    \[
        \mathbb{E}(X)=\int X\,d\mathbb{P} = \int X^+\,d\mathbb{P} - \int X^-\,d\mathbb{P},
    \]
    où $X^+=\max(X,0)$, $X^-=\max(-X,0)$.

    \textbf{Cas discret (formule avec $X(\omega)$).} Si $\Omega$ est dénombrable et que $\mathbb{P}(\{\omega\})$ est définie pour tout $\omega\in\Omega$, alors pour une v.a. $X$ intégrable,
    \[
        \mathbb{E}[X] = \sum_{\omega\in\Omega} X(\omega)\,\mathbb{P}(\{\omega\}),
    \]
    et par exemple, pour deux v.a. $X,Y$ intégrables,
    \[
        \mathbb{E}[X+Y]
        = \sum_{\omega\in\Omega} \bigl(X(\omega)+Y(\omega)\bigr)\,\mathbb{P}(\{\omega\}).
    \]

    \textbf{Propriétés} :
    \begin{itemize}
        \item Linéarité : $\mathbb{E}(aX+bY)=a\mathbb{E}(X)+b\mathbb{E}(Y)$.
        \item Monotonie : $X\le Y$ p.s. $\Rightarrow \mathbb{E}(X)\le\mathbb{E}(Y)$.
        \item Positivité : $X\ge 0$ p.s. $\Rightarrow \mathbb{E}(X)\ge 0$.
    \end{itemize}

    Lien avec les formules classiques : si $X$ admet une densité $f$ et $g$ borélienne,
    \[
        \mathbb{E}\big(g(X)\big)=\int_{\mathbb{R}} g(x) f(x)\,dx,
    \]
    ce qui est la version "Lebesgue" de LOTUS.

    \paragraph{Petit dictionnaire de notations (vue d’ensemble).}

    \begin{description}
        \item[$\Omega$] Espace des issues possibles (\emph{espace des états}, \emph{sample space}).
        \item[$\omega\in\Omega$] Une issue élémentaire, un résultat possible d’une expérience aléatoire.
        \item[$\mathcal{A}$] Tribu (\(\sigma\)-algèbre) d’événements : sous-ensemble de $\mathcal{P}(\Omega)$ stable par les opérations usuelles.
        \item[$A\in\mathcal{A}$] Un \textbf{événement}. On peut écrire $A=\{\omega : \text{``quelque propriété sur } \omega\text{''}\}$.
        \item[$\mathbb{P}$] Mesure de probabilité sur $(\Omega,\mathcal{A})$; pour $A\in\mathcal{A}$, $\mathbb{P}(A)$ est la probabilité que $A$ se réalise.
        \item[$X$] Variable aléatoire réelle, vue comme application $X:\Omega\to\mathbb{R}$.
        \item[$X(\omega)$] Valeur de la v.a. $X$ lorsque l’issue élémentaire réalisée est $\omega$.
        \item[$\{X\in B\}$] Événement $\{\omega\in\Omega : X(\omega)\in B\}$, pour un borélien $B\subset\mathbb{R}$.
        \item[$\mathbb{P}_X$] Loi (ou distribution) de $X$ : mesure image de $\mathbb{P}$ par $X$, donnée par
              $\mathbb{P}_X(B)=\mathbb{P}(X\in B)$.
        \item[$\mathbb{E}(X)$] Espérance de $X$ :
              \[
                  \mathbb{E}[X]
                  = \int_\Omega X(\omega)\,d\mathbb{P}(\omega)
                  = \int_{\mathbb{R}} x\,\mathbb{P}_X(dx)
              \]
              (et dans le cas discret : $\mathbb{E}[X]=\sum_{\omega} X(\omega)\mathbb{P}(\{\omega\})$).
    \end{description}

    \subsection{Convergences et théorèmes majeurs}

    Soient $(X_n)$ et $X$ des v.a. réelles.

    \begin{description}
        \item[Convergence presque sûre] $X_n\to X$ p.s. si
              \[
                  \mathbb{P}\big(\{\omega : X_n(\omega)\to X(\omega)\}\big)=1.
              \]
        \item[Convergence en probabilité] $X_n\to X$ en prob. si $\forall \varepsilon>0$,
              \[
                  \mathbb{P}(|X_n-X|>\varepsilon)\to 0.
              \]
        \item[Convergence en $L^1$] $X_n\to X$ en $L^1$ si $\mathbb{E}|X_n-X|\to 0$.
        \item[Convergence en loi] $X_n\to X$ en loi si $\mathbb{P}_{X_n}$ converge faiblement vers $\mathbb{P}_X$ (convergence des CDF en tout point de continuité).
    \end{description}

    Relations importantes (sans réciproque en général) :
    \[
        X_n\to X \text{ p.s. } \Rightarrow X_n\to X \text{ en prob. } \Rightarrow X_n\to X \text{ en loi},
    \]
    \[
        X_n\to X \text{ en } L^1 \Rightarrow X_n\to X \text{ en prob.}
    \]

    \textbf{Théorème de convergence monotone (Beppo-Levi)} : si $0\le X_n\uparrow X$ p.s., alors
    \[
        \mathbb{E}(X_n)\uparrow \mathbb{E}(X).
    \]

    \textbf{Théorème de convergence dominée (Lebesgue)} : si $X_n\to X$ p.s. et $\exists Y\ge 0$ intégrable tel que $|X_n|\le Y$ p.s. pour tout $n$, alors
    \[
        \mathbb{E}(X_n)\to \mathbb{E}(X),\quad \text{et}\quad \mathbb{E}(|X_n|)\le \mathbb{E}(Y).
    \]

    \subsection{Mesures produit, Fubini et Tonelli}

    Pour deux espaces probabilisés $(\Omega_1,\mathcal{A}_1,\mathbb{P}_1)$ et $(\Omega_2,\mathcal{A}_2,\mathbb{P}_2)$, il existe une mesure produit $\mathbb{P}_1\otimes\mathbb{P}_2$ sur $(\Omega_1\times\Omega_2,\mathcal{A}_1\otimes\mathcal{A}_2)$.

    Pour $f:\Omega_1\times\Omega_2\to[0,\infty]$ mesurable,
    \[
        \int f\,d(\mathbb{P}_1\otimes\mathbb{P}_2)=\int_{\Omega_2}\left(\int_{\Omega_1} f\,d\mathbb{P}_1\right)d\mathbb{P}_2
        =\int_{\Omega_1}\left(\int_{\Omega_2} f\,d\mathbb{P}_2\right)d\mathbb{P}_1
    \]
    (Tonelli pour $f\ge 0$; Fubini si $\int |f|\,d(\mathbb{P}_1\otimes\mathbb{P}_2)<\infty$).
    ...
    \begin{description}
        ...
        \item[Tail formula for expectation]
              If $X \ge 0$ a.s. with density $f_X$ on $[0,\infty)$, then
              \[
                  \mathbb{E}[X]
              ...
              \emph{Proof (very sketchy).}
              Start from the density formula and use $x = \int_0^x 1\,dt$:
              \[
                  \mathbb{E}[X]
                  = \int_0^\infty x f_X(x)\,dx
                  = \int_0^\infty \int_0^x f_X(x)\,dt\,dx.
              \]
              On the region $D = \{(t,x): 0 \le t \le x < \infty\}$, Tonelli/Fubini gives
              \[
                  \int_0^\infty \int_0^x f_X(x)\,dt\,dx
    ...
    \vspace{1.5in}
    \section{Distributions in Python} \smallskip \hrule height 2pt \smallskip

    \bigskip

    \noindent\textbf{Assume:} \texttt{import numpy as np} and \texttt{from scipy.stats import *}.
    ...
    \begin{center}
        \begin{tabular}{cc}
            ...
            \hline
            \texttt{help(stats)}               & shows documentation on \texttt{scipy.stats}               \\
            \texttt{binom.pmf(k, n, p)}        & PMF $P(X=k)$ for $X \sim \Bin(n,p)$                       \\
            \texttt{binom.cdf(x, n, p)}        & CDF $P(X \le x)$ for $X \sim \Bin(n,p)$                   \\
            \texttt{binom.ppf(a, n, p)}        & $a$th quantile for $X \sim \Bin(n,p)$                     \\
            \texttt{binom.rvs(n, p, size=r)}   & vector of $r$ i.i.d.~$\Bin(n,p)$ samples                  \\
            \texttt{geom.pmf(k, p)}            & PMF $P(X=k)$ for $X \sim \Geom(p)$                        \\
            \texttt{hypergeom.pmf(k, M, K, n)} & PMF $P(X=k)$ for hypergeometric $X$                       \\
            \texttt{nbinom.pmf(k, r, p)}       & PMF $P(X=k)$ for $X \sim \NBin(r,p)$                      \\
            \texttt{poisson.pmf(k, mu)}        & PMF $P(X=k)$ for $X \sim \Pois(\lambda)$                  \\
            \texttt{beta.pdf(x, a, b)}         & PDF $f(x)$ for $X \sim \Beta(a,b)$                        \\
            \texttt{chi2.pdf(x, n)}            & PDF $f(x)$ for $X \sim \chi^2_n$                          \\
            \texttt{expon.pdf(x, scale)}       & PDF $f(x)$ for $X \sim \Expo(\text{rate}=1/\text{scale})$ \\
            \texttt{gamma.pdf(x, a, scale)}    & PDF $f(x)$ for $X \sim \Gam(a,\text{scale})$              \\
            \texttt{lognorm.pdf(x, s, scale)}  & PDF $f(x)$ for lognormal $X$                              \\
            \texttt{norm.pdf(x, m, s)}         & PDF $f(x)$ for $X \sim \N(m,s^2)$                         \\
            \texttt{t.pdf(x, n)}               & PDF $f(x)$ for $X \sim t_n$                               \\
            \texttt{uniform.pdf(x, a, b-a)}    & PDF $f(x)$ for $X \sim \Unif(a,b)$                        \\
            \hline
        \end{tabular}
    \end{center}


    The table above gives Python/SciPy commands for working with various named
    distributions. Methods analogous to \texttt{pmf/pdf}, \texttt{cdf},
    \texttt{ppf} (quantiles), and \texttt{rvs} (random generation) exist for
    the other distributions in the table. For example, \texttt{norm.cdf},
    \texttt{norm.ppf}, and \texttt{norm.rvs} can be used to get the CDF,
    quantiles, and random generation for the Normal.  For the Multinomial,
    \texttt{multinomial.pmf} and \texttt{multinomial.rvs} can be used for
    calculating the joint PMF and generating random vectors. For the
    Multivariate Normal, \texttt{scipy.stats.multivariate\_normal} provides
    \texttt{pdf}, \texttt{cdf} (approximate), and \texttt{rvs} for joint
    densities and random vectors.

    \bigskip
    \bigskip

    \section{Distributions in C++} \smallskip \hrule height 2pt \smallskip

    \bigskip

    \noindent\textbf{Assume:}
    \begin{itemize}
        \item \texttt{\#include <boost/math/distributions.hpp>}
        \item \texttt{using namespace boost::math;}
    \end{itemize}

    \begin{center}
        % \scriptsize  % optional: keep or comment this out
        \resizebox{\linewidth}{!}{%
            \begin{tabular}{@{}ll@{}}
                \hline
                \textbf{Command}                                        & \textbf{What it does}                                   \\
                \hline
                \texttt{pdf(binomial\_distribution<>(n,p),k)}           & PMF $P(X=k)$ for $X \sim \Bin(n,p)$                     \\
                \texttt{cdf(binomial\_distribution<>(n,p),x)}           & CDF $P(X \le x)$ for $X \sim \Bin(n,p)$                 \\
                \texttt{quantile(binomial\_distribution<>(n,p),a)}      & $a$th quantile for $X \sim \Bin(n,p)$                   \\
                \texttt{pdf(geometric\_distribution<>(p),k)}            & PMF $P(X=k)$ for $X \sim \Geom(p)$                      \\
                \texttt{pdf(hypergeometric\_distribution<>(r,n,N),k)}   & PMF $P(X=k)$ for hypergeometric $X$                     \\
                \texttt{pdf(negative\_binomial\_distribution<>(r,p),k)} & PMF $P(X=k)$ for $X \sim \NBin(r,p)$                    \\
                \texttt{pdf(poisson\_distribution<>(lambda),k)}         & PMF $P(X=k)$ for $X \sim \Pois(\lambda)$                \\
                \texttt{pdf(beta\_distribution<>(a,b),x)}               & PDF $f(x)$ for $X \sim \Beta(a,b)$                      \\
                \texttt{pdf(chi\_squared\_distribution<>(n),x)}         & PDF $f(x)$ for $X \sim \chi^2_n$                        \\
                \texttt{pdf(exponential\_distribution<>(lambda),x)}     & PDF $f(x)$ for $X \sim \Expo(\lambda)$                  \\
                \texttt{pdf(gamma\_distribution<>(shape,scale),x)}      & PDF $f(x)$ for $X \sim \Gam(\text{shape},\text{scale})$ \\
                \texttt{pdf(lognormal\_distribution<>(m,s),x)}          & PDF $f(x)$ for lognormal $X$                            \\
                \texttt{pdf(normal\_distribution<>(m,s),x)}             & PDF $f(x)$ for $X \sim \N(m,s^2)$                       \\
                \texttt{pdf(students\_t\_distribution<>(n),x)}          & PDF $f(x)$ for $X \sim t_n$                             \\
                \texttt{pdf(uniform\_distribution<>(a,b),x)}            & PDF $f(x)$ for $X \sim \Unif(a,b)$                      \\
                \hline
            \end{tabular}%
        }
    \end{center}
    ...
    \noindent The generic \texttt{cdf}, \texttt{quantile}, etc.\ work for all these Boost.Math
    distribution objects. For random generation, use the C++ standard
    library, e.g.
    \begin{center}
        \footnotesize
        \texttt{std::mt19937 gen(seed); std::normal\_distribution<double> d(m,s); double x = d(gen);}
    \end{center}


    with analogous \texttt{std::binomial\_distribution}, \texttt{std::poisson\_distribution},
