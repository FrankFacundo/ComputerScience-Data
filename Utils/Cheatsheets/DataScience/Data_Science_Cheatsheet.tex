...
\begin{multicols}{3}
    ...
    \textbf{Setup.} Binary data $\mathcal{D}=\{(x_i,y_i)\}_{i=1}^N$, $y_i\in\{0,1\}$, $x_i\in\mathbb{R}^d$. Let $\tilde x_i=(1,x_i^T)^T$, $\theta=(\beta_0,\beta^T)^T$, score $z_i=\theta^T\tilde x_i$, and sigmoid $\sigma(z)=\frac{1}{1+e^{-z}}$.

    \textbf{Model.}
    $\displaystyle p_i=P(Y_i{=}1\mid x_i;\theta)=\sigma(z_i)$ and
    $\displaystyle P(Y_i{=}0\mid x_i;\theta)=1-p_i$.
    Thus $Y_i\mid x_i\sim\mathrm{Bernoulli}(p_i)$ and the decision rule is $\hat y_i=\ind\{p_i\ge a\}$ for threshold $a$ (often $0.5$).
    \vspace{.2em}

    \textbf{Likelihood / MLE.}
    $\displaystyle L(\theta)=\prod_{i=1}^N p_i^{y_i}(1-p_i)^{1-y_i}$,
    so
    $\displaystyle \ell(\theta)=\log L(\theta)=\sum_{i=1}^N\left[y_i\log p_i+(1-y_i)\log(1-p_i)\right]$.
    Training by MLE maximizes $\ell(\theta)$, equivalently minimizes the negative log-likelihood / binary cross entropy:
    \[
        J(\theta)=-\ell(\theta)=
    ...
    \textbf{Gradient.}
    With design matrix $\tilde X$ and $p=(p_1,\ldots,p_N)^T$,
    $\displaystyle \nabla_\theta J(\theta)=\tilde X^T(p-y)$.
    Batch gradient descent updates
    $\displaystyle \theta\leftarrow\theta-\eta\,\tilde X^T(p-y)$
    (or divide by $N$ if $J$ is averaged).
    \vspace{.2em}

    \textbf{Convexity / Newton.}
    $\displaystyle H=\nabla_\theta^2J=\tilde X^TW\tilde X$ where
    $W=\mathrm{diag}(p_i(1-p_i))\succeq0$, so $J$ is convex and has no local non-global minima. Newton / IRLS uses
    $\displaystyle \theta\leftarrow\theta-H^{-1}\nabla J$.
    \vspace{.2em}

    \textbf{Regularized objective.}
    Add penalties to the loss, not the exponent:
    $\displaystyle J_\lambda(\theta)=J(\theta)+\lambda||\beta||_1$ (LASSO/sparse) or
    $\displaystyle J_\lambda(\theta)=J(\theta)+\frac{\lambda}{2}||\beta||_2^2$ (Ridge/shrinkage), usually without penalizing $\beta_0$.
    Ridge adds $\lambda(0,\beta^T)^T$ to the gradient; LASSO uses subgradients / coordinate descent.
    ...
    \end{itemize}
    \textbf{Odds / logit.}
    $\displaystyle \frac{p}{1-p}=e^{\beta_0+\beta^Tx}$ and
    $\displaystyle \log\frac{p}{1-p}=\beta_0+\beta^Tx$.
    ...
    \textbf{Actor-Critic Model} - hybrid algorithm that relies on two neural networks, an actor $\pi(s,a,\theta$) which controls agent behavior  and a critic $Q(s,a,w)$ that measures how good an action is. Both run in parallel to find the optimal weights $\theta, w$ to maximize expected reward. At each step:
        ...
        \begin{itemize}[label={--},leftmargin=4mm]
            \itemsep -.4mm
            \item Stationarity - behavior does not depend on absolute time
                  \vspace{-.7mm}
                  \begin{itemize}[label={$\circ$},leftmargin=3mm]
                      \itemsep -.4mm
                      \item Weak - first two moments are time-invariant: $E[x_t]=\mu$, $\mathrm{Var}(x_t)=\gamma_0$, and $\mathrm{Cov}(x_t,x_{t-k})=\gamma_k$ depends only on lag $k$
                      \item Strong/Strict - full joint distribution is shift-invariant: $(x_{t_1},\ldots,x_{t_n})\overset{d}{=}(x_{t_1+h},\ldots,x_{t_n+h})$ for all $n,t_i,h$
                      \item Strong $+$ finite variance $\Rightarrow$ weak; weak $\nRightarrow$ strong, except for Gaussian series
                  \end{itemize}
            \item Trend - long-term rise or fall in the mean; $x_t=m(t)+u_t$ where $E[x_t]=m(t)$ and $m(t)$ is not constant, e.g. $m(t)=\beta_0+\beta_1t$
            \item Seasonality - fixed-period pattern; in $x_t=T_t+S_t+\epsilon_t$, the seasonal component satisfies $S_{t+s}=S_t$ for period $s$
            \item Cyclicality - recurring pattern without fixed period; in $x_t=T_t+C_t+\epsilon_t$, $C_{t+P_t}\approx C_t$ where the cycle length $P_t$ varies
            \item Autocorrelation - linear dependence between current and lagged values; $\rho_k=\frac{\mathrm{Cov}(x_t,x_{t-k})}{\sqrt{\mathrm{Var}(x_t)\mathrm{Var}(x_{t-k})}}$, and for stationary series $\rho_k=\gamma_k/\gamma_0$
            \item Heteroskedasticity - non-constant variance over time; $\mathrm{Var}(\epsilon_t\mid\mathcal{F}_{t-1})=\sigma_t^2$ is not constant, often appearing as volatility clustering
        \end{itemize}

        \textbf{ACF (Autocorrelation Function) / PACF (Partial Autocorrelation Function)} for weakly stationary $X_t$, $Y_t=X_t-\mu$:
        $\gamma_k=E[Y_tY_{t-k}]$, $\rho_k=\gamma_k/\gamma_0=\mathrm{ACF}(k)$.
        With $Z_k=(Y_{t-1},\ldots,Y_{t-k+1})$, $\mathrm{PACF}(k)=\mathrm{Corr}(Y_t-\Pi_{Z_k}Y_t,\,Y_{t-k}-\Pi_{Z_k}Y_{t-k})=\phi_{kk}$, the last coefficient in the best linear AR$(k)$ projection. Thus ACF contains direct and intermediate-lag associations; PACF removes the intermediate lags. AR$(p)$: PACF $=0$ for $k>p$ and ACF tails off; MA$(q)$ (Moving Average): ACF $=0$ for $k>q$ and PACF tails off; ARMA (Autoregressive Moving Average): both tail off. Approximate white-noise bounds: $\pm1.96/\sqrt{T}$.
    ...
    $\gamma$ that accounts for seasonality.
        ...
        \textbf{AR(n)} - autoregressive process of order $n$, where the current value is a linear function of its previous $n$ values:
        $\displaystyle x_t=c+\phi_1x_{t-1}+\cdots+\phi_nx_{t-n}+\epsilon_t$, with white-noise error $\epsilon_t$.
        Equivalently, $\displaystyle \phi(B)x_t=c+\epsilon_t$, where $\phi(B)=1-\phi_1B-\cdots-\phi_nB^n$. Stationarity requires the roots of $\phi(z)=0$ to lie outside the unit circle; for AR(1), $|\phi_1|<1$.
        \smallskip

        \textbf{ARIMA (Autoregressive Integrated Moving Average)} - models time series using three parameters $(p,d,q)$:
        \begin{itemize}[label={--},leftmargin=4mm]
            \itemsep -.4mm
        ...
        \textbf{SARIMA (Seasonal Autoregressive Integrated Moving Average)} - models seasonality through four additional seasonality-specific parameters: $P$, $D$, $Q$, and the season length $s$
        \smallskip

        \textbf{GARCH (Generalized Autoregressive Conditional Heteroskedasticity)} - models time-varying volatility, commonly for stationary returns.
        \textbf{Setup.} $r_t=\mu_t+\epsilon_t$, $\epsilon_t=\sqrt{h_t}z_t$, $E[z_t]=0$, Var$(z_t)=1$, and $h_t=\sigma_t^2=$ Var$(r_t\mid\mathcal{F}_{t-1})$.
        \vspace{.2em}

        \textbf{Model.} For GARCH$(p,q)$,
        $\displaystyle h_t=\omega+\sum_{i=1}^{q}\alpha_i\epsilon_{t-i}^2+\sum_{j=1}^{p}\beta_j h_{t-j}$.
        Here $q$ is the ARCH order for lagged shocks and $p$ is the GARCH order for lagged variances.
        Positivity: $\omega>0$, $\alpha_i,\beta_j\geq0$. Covariance stationarity usually requires $\sum_i\alpha_i+\sum_j\beta_j<1$.
        \vspace{.2em}

        \textbf{GARCH(1,1).}
        $\displaystyle h_t=\omega+\alpha\epsilon_{t-1}^2+\beta h_{t-1}$, with long-run variance
        $\displaystyle \bar h=\frac{\omega}{1-\alpha-\beta}$ when $\alpha+\beta<1$.
        $\alpha$ measures reaction to new shocks, $\beta$ measures persistence, and $\alpha+\beta$ measures volatility clustering.
        \vspace{.2em}

        \textbf{Likelihood / MLE.} If $z_t\sim N(0,1)$, the conditional log-likelihood is
        $\displaystyle \ell(\theta)=-\frac12\sum_{t=1}^{T}\left[\log(2\pi)+\log h_t+\frac{\epsilon_t^2}{h_t}\right]$,
        where $\theta=(\omega,\alpha_1,\ldots,\alpha_q,\beta_1,\ldots,\beta_p)$ is estimated recursively by maximizing $\ell(\theta)$.
        One-step variance forecast for GARCH(1,1): $\hat h_{t+1|t}=\omega+\alpha\epsilon_t^2+\beta h_t$.
        \smallskip
    ...
    \hat\sigma^2_{cj}=\frac{1}{N_c}\!\sum_{i:y_i=c}\!(x_{ij}-\hat\mu_{cj})^2$ (use $1/(N_c\!-\!1)$ for unbiased).
        ...
        \textcolor{white}{.}\vspace{-5mm}\\ % Add space above column
        \section{Statistics}
        \textbf{Hypothesis Test} - compare a null with an alternative using a statistic $T=T(X)$:
        \[
            H_0:\theta\in\Theta_0\ \text{(null)}
            \qquad\text{vs.}\qquad
            H_1:\theta\in\Theta_1\ \text{(alternative)}.
        \]
        For observed $t_{\mathrm{obs}}$, the \textbf{$p$-value} is the probability, assuming $H_0$, of a statistic at least as extreme:
        \[
            \begin{aligned}
                p_{\mathrm{right}}&=P_{H_0}(T\ge t_{\mathrm{obs}}),\\
                p_{\mathrm{two}}&=P_{H_0}(|T|\ge|t_{\mathrm{obs}}|)
                &&\text{(symmetric null)}.
            \end{aligned}
        \]
        At significance level $\alpha$, reject $H_0$ if $p\le\alpha$ (equivalently $T$ lies in the critical region); otherwise \emph{fail to reject} $H_0$, not \emph{accept} it. The $p$-value is not $P(H_0\mid X)$.
        \[
            \begin{aligned}
                \alpha&=P(\text{reject }H_0\mid H_0) &&\text{(Type I)},\\
                \beta&=P(\text{fail to reject }H_0\mid H_1) &&\text{(Type II)},\\
                \text{Power}&=1-\beta.
            \end{aligned}
        \]
        For fixed sample size/effect, lowering $\alpha$ generally raises $\beta$. A $(1-\alpha)$ \textbf{confidence interval} has long-run coverage $1-\alpha$; for matching two-sided procedures, reject $H_0:\theta=\theta_0$ iff $\theta_0$ is outside the interval.
