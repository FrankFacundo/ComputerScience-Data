...
TYPES = {
    ...
    "BNMH": [
        "Bticino My Home Server 1",
        "Home + Security",
    ],  # also API Home+Control  GATEWAY
    ...
    "NAPlug": [
        "thermostat relais station",
        "Energy",
    ],  # A smart thermostat exist of a thermostat and a Relais module
...
class User:
    ...
    warnings.warn(
        "The 'User' class is no longer maintained by Netatmo", DeprecationWarning
    )
...
class HomeStatus:
    ...
    def __init__(self, authData, home_id):
        ...
        postParams = {"access_token": self.getAuthToken, "home_id": home_id}
        resp = postRequest("home_status", _HOME_STATUS, postParams)
        self.resp = resp
        self.rawData = resp["body"]["home"]
        if not self.rawData:
            raise NoHome("No home %s found" % home_id)
...
class ThermostatData:
    ...
    def __init__(self, authData, home=None):
        ...
        warnings.warn(
            "The Thermostat code is not tested due to the lack of test environment.\n"
...
class WeatherStationData:
    ...
    def __init__(self, authData, home=None, station=None):
        ...
        # Weather data
        if not self.rawData:
            raise NoDevice("No weather station in any homes")
        ...
        if home and home not in self.homes:
            raise NoHome("No home with name %s" % home)
        ...
        if station:
            ...
            self.default_station = [
                ...
                if v["home_name"] == self.default_home
            ][0]
        self.modules = dict()
    ...
    def getStation(self, station=None):
        ...
        if station in self.stationIds:
            return self.stationIds[station]
        return None
    ...
    def lastData(self, station=None, exclude=0):
        ...
        if ds.get("time_utc", limit + 10) > limit:
            ...
            lastD[s["module_name"]]["When"] = (
                lastD[s["module_name"]].pop("time_utc")
                if "time_utc" in lastD[s["module_name"]]
                else time.time()
            )
        ...
        if "modules" in s:
            for module in s["modules"]:
                ...
                if ds.get("time_utc", limit + 10) > limit:
                    ...
                    lastD[module["module_name"]]["When"] = (
                        lastD[module["module_name"]].pop("time_utc")
                        if "time_utc" in lastD[module["module_name"]]
                        else time.time()
                    )
    ...
    def MinMaxTH(self, module=None, frame="last24"):
        ...
        if module and module != s["module_name"]:
            ...
            if not m:
                raise NoDevice("Can't find module %s" % module)
...
class HomeData:
    ...
    def __init__(self, authData, home=None):
        ...
        if S or C or P or E:
            ...
            if not self.cameras[self.default_home]:
    ...
    def cameraUrls(self, camera=None, home=None, cid=None):
        ...
        if camera_data:
            ...
            try:
                resp = postRequest(
                    "Camera", temp_local_url + "/command/ping", timeout=1
                )
    ...
    def presenceUrl(self, camera=None, home=None, cid=None, setting=None):
        ...
        if camera["type"] != "NOC":
            return None  # Not a presence camera
    ...
    def presenceLight(self, camera=None, home=None, cid=None, setting=None):
        ...
        if not url or setting not in ("on", "off", "auto"):
            return None
        if setting:
    ...
    def presenceSetAction(
        self,
        camera=None,
        home=None,
        cid=None,
        eventType=_PRES_DETECTION_KIND[0],
        action=2,
    ):
        return "Currently unsupported"
        if eventType not in _PRES_DETECTION_KIND or action not in _PRES_DETECTION_SETUP:
            return None
        camera = self.cameraByName(home=home, camera=camera) or self.cameraById(cid=cid)
        postParams = {
            "access_token": self.getAuthToken,
            "home_id": camera["home_id"],
            "presence_settings[presence_record_%s]"
            % eventType: _PRES_DETECTION_SETUP.index(action),
        }
...
class WelcomeData(HomeData):
    ...
    warnings.warn(
        "The 'WelcomeData' class was renamed 'HomeData' to handle new Netatmo Home capabilities",
        DeprecationWarning,
    )
    pass
...
class HomesData:
    ...
    def __init__(self, authData, home=None):
        #
        self.getAuthToken = authData.accessToken
        postParams = {"access_token": self.getAuthToken, "home_id": home}
        #
        resp = postRequest("Module", _GETHOMES_DATA, postParams)
        #        self.rawData = resp['body']['devices']
        self.rawData = resp["body"]["homes"]
        if not self.rawData:
            raise NoHome("No home %s found" % home)
        #
        if home:
...
class HomeCoach:
    ...
    def __init__(self, authData, home=None):
        ...
        # homecoach data
        if not self.rawData:
            raise NoDevice("No HomeCoach available")
    ...
    def checkNotUpdated(self, delay=3600):
        ...
        _id = res["_id"]
        ret = []
        if time.time() - res["When"] > delay:
            ret.append({_id["_id"]: "Device Not Updated"})
        return ret if ret else None
    ...
    def checkUpdated(self, delay=3600):
        ...
        ret = []
        if time.time() - res["When"] < delay:
            ret.append({_id["_id"]: "Device up-to-date"})
        return ret if ret else None
...
def postRequest(topic, url, params=None, timeout=10):
    if PYTHON3:
        ...
        if params:
            ...
            if "access_token" in params:
                req.add_header(
                    "Authorization", "Bearer %s" % params.pop("access_token")
                )
        ...
        try:
            ...
            if err.code == 403:
                logger.warning(
                    "Your current token scope do not allow access to %s" % topic
                )
            else:
                logger.error(
                    "code=%s, reason=%s, body=%s"
                    % (err.code, err.reason, err.fp.read())
                )
            return None
    else:
        if params:
        ...
        req = (
            ...
            if params
            else urllib2.Request(url=url, headers=headers)
        )
        try:
    ...
    returnedContentType = (
        resp.getheader("Content-Type") if PYTHON3 else resp.info()["Content-Type"]
    )
    return (
...
def getStationMinMaxTH(station=None, module=None, home=None):
    ...
    if module == "*":
        ...
        if not module:
            raise NoDevice("No such module %s" % module)
        else:
    ...
    if module == "*":
        ...
        if time.time() - lastD[module]["When"] > 3600:
            result = ["-", "-"]
        else:
