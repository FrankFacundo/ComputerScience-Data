...
def ffmpeg_metadata(args, log, filename):
    ...
    raw_split = output.split("    Chapter ")[0]
    raw_metadata = (raw_split).split('Input #')[1]
...
def split(args, log, output_dir, encoded_file, chapters):
    ...
    for chapter in chapters:
        ...
        chapter_name = re_sub.sub('', (args.custom_name % values).replace('/', '-').replace(':', '-').replace(' ', ''))
        if not isinstance(chapter_name, str):
            chapter_name = str(chapter_name, 'utf-8')
