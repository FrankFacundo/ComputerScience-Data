"""GatedDeltaNet — the linear-attention branch used in Qwen3.5 layers.

This is a pure PyTorch port of the reference `torch_chunk_gated_delta_rule` /
`torch_recurrent_gated_delta_rule` paths in transformers. No triton, no fla, no
causal_conv1d kernels — just `F.conv1d` and a gated-delta-rule loop.
"""
...
def apply_mask_to_padding_states(hidden_states: torch.Tensor, attention_mask) -> torch.Tensor:
    """Zero out hidden states for padding tokens (Mamba-style)."""
...
def torch_causal_conv1d_update(
    ...
    """Single-step causal conv1d that rolls the conv_state in-place."""
...
def torch_chunk_gated_delta_rule(
    ...
    Inputs are (B, T, H, D) for query/key/value and (B, T, H) for beta/g.
    Output is (B, T, H, Dv) and optionally a final recurrent state (B, H, Dk, Dv).
    """
...
def torch_recurrent_gated_delta_rule(
    ...
    """Step-by-step recurrent gated-delta-rule (used for single-token decoding)."""
    initial_dtype = query.dtype
    if use_qk_l2norm_in_kernel:
        query = l2norm(query, dim=-1, eps=1e-6)
        key = l2norm(key, dim=-1, eps=1e-6)
...
class Qwen3_5GatedDeltaNet(nn.Module):
    def __init__(self, config: Qwen3_5TextConfig, layer_idx: int):
        ...
        # QKV conv (depthwise over the full key_dim*2 + value_dim channel axis)
    ...
    def forward(
        ...
        if use_precomputed:
            ...
            if cache_params is not None:
                # F.pad with a negative left pad crops, so this yields the last
                # `conv_kernel_size` tokens regardless of seq_len.
