...
from transformers.modeling_attn_mask_utils import AttentionMaskConverter # Used in _update_causal_mask
...
def get_model() -> Qwen3ForCausalLM: # Changed return type
    ...
    print(f"Set config._attn_implementation to '{config._attn_implementation}' for custom model.")
    ...
    model, missing_keys, unexpected_keys, mismatched_keys, disk_offload_index, error_msgs = \
    ...
    model.generation_config = GenerationConfig.from_pretrained(
        path_model, trust_remote_code=True # trust_remote_code for Qwen specific gen config
    )
    ...
    if device_map_load.get("") == "cpu":
        print("Model is on CPU. No dispatch needed if only CPU.")
         ...
         print(f"Model weights loaded, target main device GPU:{device_map_load.get('')}. Moving if not already there.")
    ...
    print(f"Model loaded and dispatched. Final model device for embed_tokens: {model.model.embed_tokens.weight.device}")
    return model
...
if __name__ == "__main__":
    ...
    try:
        ...
        if isinstance(raw_tokenizer_output, torch.Tensor): # Sometimes it directly returns input_ids
            ...
            input_ids_tensor = input_ids_unmoved.to(model.device if hasattr(model, 'device') else model.model.embed_tokens.weight.device) # Get device from a model parameter
            attention_mask_tensor = attention_mask_unmoved.to(model.device if hasattr(model, 'device') else model.model.embed_tokens.weight.device)
            ...
            processed_inputs = {k: v.to(model.device if hasattr(model, 'device') else model.model.embed_tokens.weight.device) for k, v in raw_tokenizer_output.items() if isinstance(v, torch.Tensor)}
            ...
            if 'attention_mask' in processed_inputs:
                ...
                attention_mask_tensor = (input_ids_tensor != tokenizer.pad_token_id).long() # Already on device
        else:
            raise TypeError(
    ...
    target_device = model.device if hasattr(model, 'device') else model.model.embed_tokens.weight.device
    ...
    if eos_token_id is None:
        print("Warning: EOS token ID not found. Generation will stop only at max_new_tokens.")
    ...
    with torch.no_grad():
        for i in range(max_new_tokens):
            ...
            if past_key_values is None: # First iteration
                current_cache_position = torch.arange(current_loop_input_ids.shape[1], device=current_loop_input_ids.device)
                ...
                current_cache_position = torch.tensor([past_key_values.get_seq_length()], device=current_loop_input_ids.device)
            ...
            current_token_id_item = next_token_id_tensor[0, 0].item() # Assuming batch_size = 1
            
            token_text = tokenizer.decode([current_token_id_item], 
                                          skip_special_tokens=False, 
                                          clean_up_tokenization_spaces=False) # Decode token by token
            ...
            current_attention_mask = torch.cat(
                [current_attention_mask, torch.ones_like(next_token_id_tensor, dtype=torch.long, device=target_device)], 
                dim=1
            )
    ...
    generated_ids_tensor_trimmed = torch.tensor([generated_token_ids_collector], dtype=torch.long, device=target_device)
