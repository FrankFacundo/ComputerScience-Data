...
def download_weights_from_hf_specific(
    ...
    raise RuntimeError("Hub downloads are not supported in the Torch-only local runtime.")
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    if attn_weights.shape[-1] != value_states.shape[-2]:
        raise RuntimeError(
            f"attention/value length mismatch layer={getattr(module, 'layer_idx', None)} "
            f"q={tuple(query.shape)} k={tuple(key_states.shape)} v={tuple(value_states.shape)} "
        ...
        **kwargs,
        ...
        for _ in range(max_new_tokens - 1):
            ...
            outputs = self(
                ...
                attention_mask=attention_mask,
                past_key_values=past_key_values,
                cache_position=torch.tensor([past_key_values.get_seq_length()], device=attention_mask.device),
                use_cache=True,
                output_hidden_states=output_hidden_states,
...
class Qwen3TTSTalkerForConditionalGeneration(Qwen3TTSTalkerTextPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attention_mask is not None:
            if cache_position is not None and input_ids is not None and input_ids.shape[1] == 1 and cache_position[0] != 0:
    ...
    def generate(
        ...
        **kwargs,
        ...
        hidden_states = [outputs.hidden_states] if output_hidden_states else None
        generated = []
        suppress = list(suppress_tokens or [])
        if eos_token_id is not None and min_new_tokens > 0 and eos_token_id not in suppress:
            suppress_with_min = suppress + [eos_token_id]
        else:
            suppress_with_min = suppress
        next_token = sample_token(
            outputs.logits[:, -1, :],
            do_sample=do_sample,
            temperature=temperature,
            ...
            generated_ids=None,
            suppress_tokens=suppress_with_min,
        )
        next_token = next_token.reshape(next_token.shape[0], -1)[:, -1:]
        generated.append(next_token)

        for step in range(max(max_new_tokens - 1, 0)):
            attention_mask = torch.cat(
                [attention_mask, torch.ones((attention_mask.shape[0], 1), dtype=attention_mask.dtype, device=attention_mask.device)],
                dim=1,
            )
            ...
            next_token = sample_token(
                ...
                top_p=top_p,
                repetition_penalty=repetition_penalty,
                generated_ids=torch.cat(generated, dim=-1) if generated else None,
                suppress_tokens=suppress_now,
            )
            next_token = next_token.reshape(next_token.shape[0], -1)[:, -1:]
            generated.append(next_token)
            if eos_token_id is not None and bool((next_token == eos_token_id).all()) and len(generated) >= min_new_tokens:
                break
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        raise RuntimeError("Use qwen_tts.Qwen3TTSModel.from_pretrained for local Torch-only loading.")
