...
from ..audio_features import EncodecFeatureExtractor
...
from ..local_models import resolve_local_model_path
...
class Qwen3TTSTokenizer:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs) -> "Qwen3TTSTokenizer":
        ...
        inst.feature_extractor = EncodecFeatureExtractor.from_pretrained(pretrained_model_name_or_path)
        config = Qwen3TTSTokenizerV2Config.from_pretrained(pretrained_model_name_or_path)
        attn_implementation = kwargs.pop("attn_implementation", "eager")
        config._attn_implementation = attn_implementation
        config.encoder_config._attn_implementation = attn_implementation
        ...
        if isinstance(device_map, dict) and "" in device_map:
            ...
            device = device_map
        inst.model = Qwen3TTSTokenizerV2Model(config)
        if dtype is not None:
            inst.model = inst.model.to(dtype=dtype)
        state = load_file(os.path.join(pretrained_model_name_or_path, "model.safetensors"), device="cpu")
        inst.model.load_state_dict(state, strict=True)
        inst.model = inst.model.to(device).eval()
